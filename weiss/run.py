...
'''
    Funcion que realiza el ONE HOT ENCODING
'''
...
data_df = pd.read_csv('data/unbalaced_20_80_dataset.csv')

'''
Eliminamos las columnas no necesarias como indica el Paper
'''
data_df = data_df.drop(columns=['Flow Byts/s', 'Flow Pkts/s'])
data_df = data_df.drop(
    columns=['Fwd Byts/b Avg', 'Fwd Pkts/b Avg', 'Fwd Blk Rate Avg', 'Bwd Byts/b Avg', 'Bwd Pkts/b Avg',
             'Bwd Blk Rate Avg'])
data_df = data_df.drop(columns=['Fwd URG Flags', 'Bwd URG Flags'])


'''
Ejecutamos el One-Hot-Encoding
'''
...
#Eliminamos la columna de otras ips del one hot encoding
